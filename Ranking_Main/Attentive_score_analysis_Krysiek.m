...
file_loc = 'C:\Users\pxb08145\OneDrive - University of Strathclyde\Documents\GitHub\Autism_Gameplay\adjs\adj_extendplate\'; %\adj_krysiek\'; % should match zone type
...
%             L = adj2L_snap2zones(adj,num);
        ...
        pert = max(abs(V(4:7,I(1)))./sum(abs(V(4:7,I(1))))) - min(abs(V(4:7,I(1)))./sum(abs(V(4:7,I(1)))));
%         pert=min(abs(V(4:7,I(1)))./sum(abs(V(:,I(1)))))-
%         pert=max(abs(V([1,2,3,8,9,10,11,12,13,14,15,16],I(1)))./sum(abs(V(:,I(1)))));
...
if strcmp(type,'plates')
    if strcmp(label,'accurate')
        save('extend_attentive_accurate_Krysiek.mat','ranked','nam_save')
    else
        save('extend_attentive_Krysiek.mat','ranked','nam_save')
    end
elseif strcmp(type,'inter')
    save('extend_attentive_inter_Krysiek.mat','ranked','nam_save')
end
