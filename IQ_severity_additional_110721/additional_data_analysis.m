...
subj_grp = 'TD';
sev_choice = '';        % set as empty string or 1,2,3
num=16;                 % accurate = 16, snap-to = 12
option = 1;             % 1 = n_swipes, 2 = sharing score
destination = 'food';   % n_swipes for 'plates' or 'food' destinations
...
for kk = 1 : n_meas      % different scores (e.g. mood)
    [nam_save,num,saved,ranked] = load_dataset(option,num,folder_loc);
    tab_sev = readtable([floc,'\eCRF.csv']);

    [n_swipes,score,sets,asdname_save,titlename,savename,map,type] = analyse_subjects(kk,tab_sev,file_loc,subj_grp,nam_save,num,destination,measure);
    ...
    plot_results(subj_grp,sev_choice,all_sets,sets,grps,option,destination,num)
...
function [nam_save,num,saved,ranked] = load_dataset(option,num,folder_loc)
    if option == 1      % volume
        if num == 16
            load([folder_loc,'\Ranking_Correlations_110721\Data\OBJ_end_accurate_bi.mat'],'nam_save','num','saved','ranked')
        elseif num == 12
            load([folder_loc,'\Ranking_Correlations_110721\Data\OBJ_end_12zones.mat'],'nam_save','num','saved','ranked')
        end
    elseif option == 2  % proportion
        if num == 16
            load([folder_loc,'\Ranking_Correlations_110721\Data\OBJ_end_accurate_proport_bi.mat'],'nam_save','num','saved','ranked')
        elseif num == 12
            load([folder_loc,'\Ranking_Correlations_110721\Data\OBJ_end_12zones_proport.mat'],'nam_save','num','saved','ranked')
        end
    end
end
...
function [n_swipes,score,sets,asdname_save,titlename,savename,map,type] = analyse_subjects(kk,tab_sev,file_loc,subj_grp,nam_save,num,desination,measure)
...
function [ranked,n_swipes,map,months] = clean_results(ranked,n_swipes,saved,map,months)
    ...
    tmp_swipes = n_swipes; % not sure what these lines achieve
...
function [] = plot_results(subj_grp,sev_choice,all_sets,sets,grps,option,destination,num)
