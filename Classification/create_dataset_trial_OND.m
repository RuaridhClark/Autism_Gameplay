...
function dataset_trial = create_data()
    ...
    for i = 1 : size(nam_save,2)
        ...
        if max(saved(:,I))>0 && ~strcmp(subject_details{I,5},'ONDE') && ~strcmp(subject_details{I,5},'TD') %&& ~strcmp(subject_details{I,5},'OND') %&& strcmp(subject_details{I,4},'Male') 
            age = [age;subject_details{I,2}*12 + subject_details{I,3}];
            
            if strcmp(subject_details{I,5},'OND')
                if ismember(i,sets_OND{1})
                    cat = [cat;{'ADHD'}];
                elseif ismember(i,sets_OND{2})
                    cat = [cat;{'Language'}];
                elseif ismember(i,sets_OND{3})
                    cat = [cat;{'Other'}];
                elseif ismember(i,sets_OND{4})
                    cat = [cat;{'Down'}];
                else
                    cat = [cat;{'Other'}];
                end
