...
%         h = kstest(diagsA(zones(2),sets{num}))
    iter=iter+1;
    vals = n_swipes(1,sets{num});%diagsA(zones(2),sets{num});
...
%     pval = kruskalwallis([diagsA(zones(2),sets{num(1)}),diagsA(zones(2),sets{num(2)})],len_rankeds,'off');
    pval = kruskalwallis([n_swipes(1,sets{num(1)}),n_swipes(1,sets{num(2)})],len_rankeds,'off');
    save_p(1,j) = pval;
end
...
for num = 1 : 4
    ...
    x=months(sets{num});
    y=n_swipes(1,sets{num})';%diagsA(2,sets{num})';
    [pf,S] = polyfit(x,y,2);
