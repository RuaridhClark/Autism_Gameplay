% identify swipes is the same just tidied!!!!!
% check for mismatches in the touch count (0 for start etc)
function [tC_save,nam_save] = fix_tapCount_errors(rng_strt,rng_end)
...
%     parpool
    ...
    for f = rng_strt:rng_end %1:length(D) % avoid using the first ones?
...
%                 end
                ...
                starts = find(tab.touch_phase==0);
                ends = find(tab.touch_phase==3);
                ...
                for j = 1 : length(valid)
                    ...
                    if tC(vj)==0
                        prevj=j;
                        current_Phase = tab.touch_phase(vj);
                        nm_srch = 750;
                        dist_all=ones(nm_srch,1)*10000;
                        tC_check=[];
                        for jj = 1:nm_srch          % check up to nm_srch prior touches for best match
                            j=j-1;%tj=nm_srch-jj;           % dodgy coding but goes back through the previous nm_srch number of touches
                        ...
                        while ~ismember(tC(valid(prevj-I)),viable) || tab.touch_phase(valid(prevj-I))==3 % check if swipe has already ended
                            dist_all(I)=10000;
                            [~,I]=min(dist_all);
                        ...
                        if tab.touch_phase(valid(prevj))==3
...
function [valid,invalid,starts,ends] = check_validity(tab,starts,ends)
    ...
    for j = 2 : length(tab.x)           % for each touch recording
        if (tab.x(j-1)-tab.x(j))== 0 && (tab.y(j-1)-tab.y(j))==0 ...
            && abs(tab.touch_phase(j)-tab.touch_phase(j-1))~=3 % Check for repeats and only allow them if a touch is ending
