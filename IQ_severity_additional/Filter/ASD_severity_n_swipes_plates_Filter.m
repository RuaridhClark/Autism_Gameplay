...
map = [];
    ...
    for i = 1:height(tab_sev)
        ...
        if isfile([file_loc,file_id]) && ~isempty(severity)
            if tab_sev.additional_patient_data__cooperativity(i)>1 && tab_sev.recording_day_data__distractibility(i)>1 && tab_sev.recording_day_data__interest_in_tablet_games(i)>1%
